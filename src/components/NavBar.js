...
function NavBar(props) {
    ...
    return ( <
        ...
        <
        div className = "relative z-50 inline-flex  group"
        onClick = {
        ...
        /div> { <
            ...
            div className = { `flex flex-col  overflow-hidden ${isPressed ? 'opacity-100 mt-8 w-1/4' : 'opacity-0 ml-20 w-0 -mt-10'} justify-between transition-all duration-500  h-50 bg-blue-500  gap-3 absolute  p-3 z-40 -ml-48 rounded-xl` } >
                <
                div className = ' text-white absolute -inset-x-2 -inset-y-2 bg-black font-bold rounded-full w-6 pt-0.5 text-center left-0.5 top-1 cursor-pointer h-fit border-2'
            onClick = {
                    () => setIsPressed(!isPressed) } > < p className = '-mt-1' > x < /p></div >
                <
            ...
            type = 'email'
            className = ' outline-none p-2 pl-4 w-full rounded-2xl' > < /input> <
                button onClick = {
                    () => {
        ...
        span className = "relative invisible" > Log0ut < /span> <
        /a> <
        /div> <
        /div> <
        /div> <
